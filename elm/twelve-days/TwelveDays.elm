...
matchLyric : Int -> Int -> ( Int, String ) -> Maybe String
matchLyric start stop line =
    if Tuple.first line >= start && Tuple.first line <= stop then
        Just (Tuple.second line)
...
recite start stop =
    song
        |> List.filterMap (matchLyric start stop)
